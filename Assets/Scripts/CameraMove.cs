...
public class CameraMove : MonoBehaviour {
	...
	void Update () {
        ...
        Debug.Log("---------");
        if (m.transform.position.y < 15  && player.transform.position.y > m.transform.position.y && player.transform.position.y % 10 >= 9 || player.transform.position.y % 10 <= -9) // moving up to next screen
        {
